...
class ContrastiveLawLFM(nn.Module):
    def __init__(self, config, gpu_list, *args, **params):
        ...
        self.hidden_size = self.LFM.config.hidden_size
        self.cross_attention = nn.MultiheadAttention(self.hidden_size, 1)
        self.outlinear = nn.Linear(self.hidden_size * 2, 1)
    ...
    def compute_score(self, lhid, fhid):
        # lhid: cand_num, law_len, hidden_size
        # fhid: batch, fact_len, hidden_size
        batch, cand_num = fhid.shape[0], lhid.shape[0]
        lhids = lhid.unsqueeze(0).repeat(batch, 1, 1, 1).view(batch * cand_num, lhid.shape[1], self.hidden_size) # batch * cand_num, law_len, hidden_size
        fhids = fhid.unsqueeze(1).repeat(1, cand_num, 1, 1).view(batch * cand_num, fhid.shape[1], self.hidden_size) # batch * cand_num, f_len, hidden_size
    ...
    def compute_score_dense(self, lhid, fhid):
        # lhid: cand_num, law_len, hidden_size
        # fhid: batch, fact_len, hidden_size
        batch, cand_num = fhid.shape[0], lhid.shape[0]
        law_rep = self.pooler(lhid) # cand_num, hidden_size
        fact_rep = self.pooler(fhid) # batch, hidden_size
        law_rep_ex = law_rep.unsqueeze(0).repeat(batch, 1, 1)
        fact_rep_ex = fact_rep.unsqueeze(1).repeat(1, cand_num, 1)
        feature = torch.cat([law_rep_ex, fact_rep_ex], dim = 2)
        score = self.outlinear(feature).squeeze(2) # batch, cand_num
        return score
    ...
    def forward(self, data, config, gpu_list, acc_result, mode):
        ...
        match_score = self.compute_score_dense(law_hidden, fact_hiddens)
