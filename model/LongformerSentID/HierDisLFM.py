...
def create_sent_ids_from_input_ids(input_ids, padding_idx):
    """
    Replace non-padding symbols with their position numbers. Position numbers begin at padding_idx+1. Padding symbols
    are ignored. This is modified from fairseq's `utils.make_positions`.
    ...
    # The series of casts and type-conversions here are carefully balanced to both work with ONNX export and XLA.
    mask = input_ids.ne(padding_idx).int()
    period = ((input_ids == 511) | (input_ids < 100)).int()
    incremental_indices = torch.cumsum(period, dim=1).type_as(period) * mask
    # incremental_indices = torch.cumsum(mask, dim=1).type_as(mask) * mask
...
class LongformerEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.sentid_num = 300
        self.sentid_embeddings = nn.Embedding(self.sentid_num, config.hidden_size)
    ...
    def forward(self, input_ids=None, token_type_ids=None, sent_ids=None, position_ids=None, inputs_embeds=None):
        ...
        if sent_ids is None:
            sent_ids = create_sent_ids_from_input_ids(input_ids, self.padding_idx)
        # print(sent_ids.tolist())
        # gg
        sent_ids[sent_ids >= self.sentid_num] = self.sentid_num - 1
        sent_ids_emb = self.sentid_embeddings(sent_ids)
        if input_ids is not None:
        ...
        embeddings = sent_ids_emb + inputs_embeds + position_embeddings + token_type_embeddings
...
class LongformerLayer(nn.Module):
    def __init__(self, config, layer_id=0):
        ...
        self.seq_len_dim = 1
        self.layer_id = layer_id
        if self.layer_id in {2, 5, 8}:
            self.max_chunk_num = 50
            self.chunk_emb = nn.Embedding(self.max_chunk_num, config.hidden_size)
    ...
    def forward(
        ...
        if self.layer_id in {2, 5, 8}:
            # sent_pos: batch, sent_num
            index = torch.arange(layer_output.shape[0]).unsqueeze(0).repeat(1, sent_pos.shape[1]).to(sent_pos.device)
            sent_vec = layer_output[index, sent_pos] * (sent_pos > 0).unsqueeze(2) # batch, sent_num, hidden_size
            cos_sim = self.cos_sim(sent_vec[:,:-1], sent_vec[:,1:]) # batch, sent_num - 1
            seg = cos_sim < 0.1 # batch, sent_num - 1
            segid = torch.cat([torch.zeros(sent_pos.shape[0]).unsqueeze(1).to(sent_vec.device), torch.cumsum(seg, dim=1)], dim=1) # batch, sent_num
            print(segid)
            segid[segid >= self.max_chunk_num] = self.max_chunk_num - 1
            segemb = self.chunk_emb(segid) * (sent_pos > 0).unsqueeze(2) # batch, sent_num, hidden_size
            layer_output[index] = layer_output[index] + segemb
...
class HiDisLFMModel(LongformerPreTrainedModel):
    """
    ...
    def forward(
        ...
        embedding_output = self.embeddings(
            input_ids=input_ids, position_ids=position_ids, sent_ids=sent_ids, token_type_ids=token_type_ids, inputs_embeds=inputs_embeds
        )
...
class HiDisLFMForMaskedLM(LongformerPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.longformer = HiDisLFMModel(config, add_pooling_layer=False)
